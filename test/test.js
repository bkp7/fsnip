...
describe('unit tests', function () {
  it('setInputType 1', function () {
    assert.strictEqual(setInputType({}, 'invalidType'), false)
  })
  it('setInputType 2', function () {
    assert.strictEqual(setInputType({}, ''), false)
  })
  it('setInputType 3', function () {
    assert.strictEqual(setInputType({ type: '' }, ''), false)
  })
  it('setInputType 4', function () {
    assert.strictEqual(setInputType({ type: 'something' }, 'somethingElse'), false)
  })
  it('minimizeJsonProperty', function () {
    let testJson = { 'first': true }
    minimizeJsonProperty(testJson, 'first')
    assert.deepStrictEqual(testJson, { 'first': true })
  })
})
  ...
  for (let i = 0; i < tests.length; i++) {
    it(tests[i].name, function (done) {
      ...
      exec(tests[i].cmd, function (error, stdout) {
        ...
        assert.strictEqual(stdout.toString(), fs.readFileSync(path.resolve('./test/', tests[i].resultFile)).toString())
        done()
      })
    })
  }
  for (let i = 0; i < errorTests.length; i++) {
    it(errorTests[i].name, function (done) {
      ...
      exec(errorTests[i].cmd, function (error, stdout, stderr) {
        assert.ifError(error)
        assert.strictEqual(stderr.toString(), errorTests[i].stderr)
        done()
      })
    })
  }
